  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      if (reason === DisconnectReason.loggedOut) {
        ...
        if (usePairingCode) {
          // When using pairing code, QR timeouts are expected — reconnect
          // so the user has more time to enter the code.
          console.log('\n⟳ QR timed out but pairing code is active — reconnecting...');
